// Sarh — register the two issuer schemas + cred defs against ACA-Py.
//
...
//   ACA_PY_ADMIN_API_KEY      (default sarh-dev-admin-key)
//   SARH_REPO_ROOT         (default = pwd at script start)
...
const SCHEMAS: SchemaSpec[] = [
  {
    ...
    cred_def_tag: 'sarh-digital-id-v1',
  ...
  },
  {
    ...
    cred_def_tag: 'sarh-property-deed-v1',
    support_revocation: true,
  },
];

const ADMIN_URL = (process.env.ACA_PY_ADMIN_URL ?? 'http://localhost:8021').replace(/\/+$/, '');
const ADMIN_KEY = process.env.ACA_PY_ADMIN_API_KEY ?? 'sarh-dev-admin-key';
...
async function main() {
  ...
  for (const spec of SCHEMAS) {
    ...
    result[`SARH_${spec.name.toUpperCase()}_SCHEMA_ID`] = schemaId;
    result[`SARH_${spec.name.toUpperCase()}_CRED_DEF_ID`] = credDefId;
  }
  ...
  const envOut: Record<string, string> = {
    ...
    ACA_PY_DIGITAL_ID_SCHEMA_ID: result['SARH_DIGITALIDSCHEMA_SCHEMA_ID'],
    ACA_PY_DIGITAL_ID_CRED_DEF_ID: result['SARH_DIGITALIDSCHEMA_CRED_DEF_ID'],
    ACA_PY_PROPERTY_DEED_SCHEMA_ID: result['SARH_PROPERTYDEEDSCHEMA_SCHEMA_ID'],
    ACA_PY_PROPERTY_DEED_CRED_DEF_ID: result['SARH_PROPERTYDEEDSCHEMA_CRED_DEF_ID'],
  };

  const repoRoot = process.env.SARH_REPO_ROOT ?? process.cwd();
