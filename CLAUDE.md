...
- **Database**: Local **SQL Server 2019/2022** with `geography` for geometry, full-text catalog (Arabic) for name search, `INSTEAD OF` triggers for the append-only audit log. The authoritative schema is the numbered T-SQL in `infra/mssql/migrations/000–044.sql`.
  - **Primary bootstrap = EF Core migrations** (framework-driven, works on any PC with the .NET 8 SDK + a reachable SQL Server, no `sqlcmd`/manual `.sql`). Each numbered `.sql` is **embedded into the assembly** (`<EmbeddedResource>` in `Sarh.Api.csproj`, logical-name prefix `SarhSql.`) and mapped 1:1 to an EF migration class (`apps/api-dotnet/Migrations/SarhSchemaMigrations.cs`) which acts as the ordered registry; `EfDatabaseBootstrapper` then applies each pending file's batches **directly** (autocommit, `GO`-split, stripping `USE`/`CREATE`/`ALTER DATABASE` so it is database-name-agnostic) and records each in `__EFMigrationsHistory`. The bootstrapper (`apps/api-dotnet/Data/EfDatabaseBootstrapper.cs`) ensures the `sarh_app` login + target DB (`Arabic_CI_AS`), **baselines** an existing sqlcmd-built DB (records all migrations as applied — no destructive re-run), then applies pending. Migrations run on a **privileged Windows-auth connection** (`MigrationConnection.cs`); the runtime `sarh_app` login cannot run DDL. Run it via: app-startup auto-migrate (`Sarh:AutoMigrate`, on by default), `dotnet run -- --migrate` (provision then exit), or `pnpm db:migrate:ef` / `pnpm db:reset:ef` (`scripts/db/migrate-ef.ps1`). Note: the bootstrapper applies the embedded SQL **directly** (autocommit, `GO`-split) and stamps `__EFMigrationsHistory`; it does **not** use EF's `MigrateAsync` (which can't replay 45 raw-SQL batch files), so `dotnet ef database update` is not the apply path. To add schema: add `infra/mssql/migrations/NNN_*.sql`, then add a matching `Migration` class in `SarhSchemaMigrations.cs`.
  - **Legacy path** (still works): `pnpm db:reset` → `scripts/db/run-migrations.ps1` (sqlcmd-based, Windows auth).
