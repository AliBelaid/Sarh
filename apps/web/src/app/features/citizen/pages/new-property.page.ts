          ...
          <div class="card">
            ...
            <div class="field" style="margin-top: 14px;">
              ...
              <input id="docArea" type="number" step="0.01" min="0.01" [(ngModel)]="documentedAreaSqm" name="documentedAreaSqm"
                     dir="ltr" [disabled]="busy()" placeholder="مثال: ٣٦٠" />
              @if (areaMismatch()) {
                <p class="hint warn-hint">⚠ المساحة حسب الأوراق تختلف عن المحسوبة من الحدود بنسبة {{ areaDiffPct() | number: '1.0-1' }}% — سيُراجعها الموظف.</p>
              }
            </div>
          </div>
...
export class NewPropertyPage implements AfterViewInit, OnDestroy {
  ...
  // Divergence between the paper-deed area and the polygon-measured area.
  // A soft check (≥10% flags a reviewer warning) — never blocks submission,
  // since legacy paper measurements are routinely imprecise.
