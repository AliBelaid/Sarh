...
//   - Error decoder that turns the SijilliException envelope into a
//     SijilliApiError so callers can show messageAr directly.
class SijilliApiClient {
  final Dio dio;
  final FlutterSecureStorage storage;

  SijilliApiClient({required this.dio, required this.storage});

  static SijilliApiClient build({required String baseUrl}) {
    ...
    return SijilliApiClient(dio: dio, storage: storage);
  }

  Future<String?> readToken() => storage.read(key: 'sijilli_jwt');
  Future<void> writeToken(String value) =>
      storage.write(key: 'sijilli_jwt', value: value);
  Future<void> clearToken() => storage.delete(key: 'sijilli_jwt');
}
...
class _DemoInterceptor extends Interceptor {
  ...
  Future<void> onRequest(
    ...
    final token = await storage.read(key: 'sijilli_jwt');
    if (token != _demoToken) {
...
class _ErrorInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    if (response != null && response.data is Map) {
      final mapped = SijilliApiError.fromJson(
...
final apiClientProvider = Provider<SijilliApiClient>((ref) {
  // The actual baseUrl comes from --dart-define=SIJILLI_API_URL=...
  // Fallback is the local dev API.
  const fallback = 'http://10.0.2.2:3000/api/v1';
  const baseUrl = String.fromEnvironment('SIJILLI_API_URL', defaultValue: fallback);
  return SijilliApiClient.build(baseUrl: baseUrl);
});
