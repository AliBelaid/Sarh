// Renders docs/project/08-data-and-class-models.ar.md into a single RTL PDF:
// docs/project/Sarh-Data-Class-Models.pdf — via globally-installed Puppeteer.
// Same self-contained md→html + CSS as docs/print-blockchain-review.mjs,
// resolving ../diagrams/*.png figures against the docs/project base.
...
const CHAPTER = '08-data-and-class-models.ar.md';
...
function inline(text) {
  ...
  // Guard: only substitute placeholders we created — literal " 1 " in prose passes through.
...
<section class="title-page">
  ...
  <h1 class="title">توثيق النمذجة</h1>
  <p class="subtitle">المخطط المفاهيمي · المخطط المنطقي (UML) · مخطط الفئات · قاموس البيانات</p>
...
<style>
  ...
  * { box-sizing: border-box; }
  body { font-family: "Segoe UI", "Tahoma", "Traditional Arabic", sans-serif; color: #0F172A; line-height: 1.7; font-size: 12pt; }
  ...
  .title-page .title { font-size: 30pt; margin: 8px 0; color:#0F172A; }
  ...
  h4 { font-size: 12pt; color:#0891B2; margin-top: 14px; margin-bottom: 4px; }
  p { margin: 7px 0; }
  ul, ol { margin: 7px 24px 7px 0; padding: 0 18px 0 0; }
  li { margin: 3px 0; }
  ...
  code { font-family: "Consolas","Courier New",monospace; background:#f1f5f9; padding:1px 4px; border-radius:4px; font-size:9.5pt; direction:ltr; unicode-bidi: embed; }
...
const renderFile = join(projectDir, '.render-model.html');
...
const pdfPath = join(projectDir, 'Sarh-Data-Class-Models.pdf');
...
try {
  ...
  await page.pdf({
    ...
    footerTemplate: '<div style="width:100%;text-align:center;font-size:9px;color:#94a3b8;">صَرح · توثيق النمذجة · <span class="pageNumber"></span> / <span class="totalPages"></span></div>',
  ...
  if (process.argv.includes('--shots')) {
    ...
    await page.screenshot({ path: join(projectDir, '.model-preview-full.png'), fullPage: true });
    console.log('Preview PNG written: .model-preview-full.png');
  }
} finally {
  await browser.close();
