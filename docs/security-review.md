# Sijilli — Security Review Checklist
...
| Rate limit — `/auth/*` | 10r/m steady, burst 5 | `for i in $(seq 1 50); do curl -s -o /dev/null -w '%{http_code}\n' https://officer.sijilli.ly/api/v1/auth/login; done \| sort \| uniq -c` should show 429s after burst. | |
...
| HSTS preload | `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload` on every public host | `curl -I https://sijilli.ly \| grep -i strict-transport`. Repeat for all 5 hosts. | |
| TLS profile | TLS 1.2 + 1.3 only; no RC4/3DES | `nmap --script ssl-enum-ciphers -p 443 sijilli.ly` shows A grade equivalent. | |
| HTTP → HTTPS redirect | Every plain HTTP request 301s to HTTPS | `curl -I http://sijilli.ly` → `301`, `Location: https://...`. | |
| CSP header | Present on every host, no `unsafe-eval`, no `*` in `script-src` | `curl -I https://officer.sijilli.ly \| grep -i content-security-policy`. | |
| `X-Frame-Options: DENY` | Set on every web vhost | `curl -I https://admin.sijilli.ly \| grep -i x-frame-options`. | |
...
| QR verifier | QR on every deed resolves to verify.sijilli.ly/<code> | Decode a sample with `zbarimg` → URL matches. | |
...
| Non-root in API container | `docker exec sijilli-api id` returns non-zero UID | `docker exec sijilli-api id -u`. | |
